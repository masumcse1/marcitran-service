...
public class PatientResource {
	...
	private static Logger logger = Logger.getLogger(PatientResource.class);
	...
	@RequestMapping(value = "/getAllPatients/{status}", method = RequestMethod.GET)
	public ResponseEntity<?> getAllPatients(@PathVariable("status") final Integer status,
											@RequestParam(defaultValue = "0") Integer pageNumber) {
		Sort sort = new Sort(Sort.Direction.DESC, "id");
		Pageable pageable = new PageRequest(pageNumber, 10, sort);
		List<Patients> patientsList = patientService.getAllPatients(status, pageable);
	...
	public ResponseEntity<?> uploadPatientDocument(@RequestBody PatientDocumentDTO dto) {
		...
		return new ResponseEntity<>(new Response(StatusType.OK, MapperUtils.mapDocumentToDTO(document)), HttpStatus.OK);
	}
	...
	public ResponseEntity<?> getDocumentsByPatient(@RequestBody PatientDocumentDTO dto) {
		...
		return new ResponseEntity<>(new Response(StatusType.OK, patientService.getDocumentsByPatient(dto)), HttpStatus.OK);
	}
