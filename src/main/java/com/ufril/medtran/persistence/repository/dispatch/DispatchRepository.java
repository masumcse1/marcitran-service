...
public interface DispatchRepository extends JpaRepository<Dispatches, Integer> {
    @Query("SELECT new " +
            ...
            "WHERE (:employeeId IS NULL OR l.shift.id IN (SELECT m.shiftID.id FROM ShiftCrewMembers m WHERE m.employeeID.id = :employeeId)) " +
    ...
    List<DispatchDTO> getAllDispatch(@Param("status") Integer status,
    ...
    List<CallsPerDayNightDTO> getCallsPerDayNightSplit(@Param("startDate") Date startDate, @Param("endDate") Date endDate);

    @Query(value = "SELECT NEW com.ufril.medtran.dto.dispatch.CallsPerVehicleDTO(v.callSign, COUNT(d)) " +
            "FROM DispatchLogs d " +
            "JOIN d.shift.vehicle v " +
            "WHERE d.dispatch.createdDate BETWEEN :startDate AND :endDate " +
            "GROUP BY v.callSign")
    List<CallsPerVehicleDTO> countCallsPerVehicle(@Param("startDate") Date startDate, @Param("endDate") Date endDate);

    @Query(value = "SELECT new com.ufril.medtran.dto.dispatch.CallsByDispatcherDTO(d.createdBy, COUNT(d)) " +
            "FROM Dispatches d " +
            "WHERE d.createdDate BETWEEN :startDate AND :endDate " +
            "GROUP BY d.createdBy")
    List<CallsByDispatcherDTO> getCallsByDispatcherCrewMember(@Param("startDate") Date startDate, @Param("endDate") Date endDate);
}
