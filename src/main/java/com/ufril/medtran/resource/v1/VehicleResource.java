...
public class VehicleResource {
    ...
    @RequestMapping(
            value = "/vehicle/getAllVehicles",
            method = RequestMethod.GET
    )
    public ResponseEntity<?> getAllVehicle(@RequestParam(defaultValue = "0") Integer pageNumber) {
        Sort sort = new Sort(Sort.Direction.DESC, "id");
        Pageable pageable = new PageRequest(pageNumber,10, sort);
        List<Vehicles> vehicleList = vehicleService.getAllVehicles(pageable);
    ...
    public ResponseEntity<?> getVehicleById(@PathVariable("id") final int id) {
        return new ResponseEntity<>(new Response(StatusType.OK, vehicleService.getVehicleById(id)), HttpStatus.OK);
    }
